...
export async function create(req: Request, res: Response, next: NextFunction) {
  try { res.status(201).json(await focusService.createSession(req.user!.sub, req.body)); } catch (err) { next(err); }
}

export async function update(req: Request, res: Response, next: NextFunction) {
  try { res.json(await focusService.updateSession(req.user!.sub, req.params.id as string, req.body)); } catch (err) { next(err); }
}

export async function complete(req: Request, res: Response, next: NextFunction) {
  try { res.json(await focusService.completeSession(req.user!.sub, req.params.id as string)); } catch (err) { next(err); }
}

export async function cancel(req: Request, res: Response, next: NextFunction) {
  try { res.json(await focusService.cancelSession(req.user!.sub, req.params.id as string)); } catch (err) { next(err); }
}
