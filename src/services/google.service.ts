...
type GoogleOAuthState = {
  ...
  returnTo: string;
  // Opaque random token generated by the frontend popup flow. Embedded in the
  // signed OAuth state so the callback page can echo it back via postMessage()
  // and the opener can verify the message was from its own auth attempt.
  nonce?: string;
};
...
export function buildGoogleAuthUrl(
  ...
  const state = signGoogleState({ purpose, returnTo, nonce: nonce ?? undefined });
 ...
 * controller can render an error postMessage even when the overall flow fails.
 * Never throws — falls back to signin defaults when the state is invalid.
 */
export function getGoogleStateContext(state: string): {
  purpose: GoogleAuthPurpose;
  nonce: string;
} {
  try {
    const parsed = verifyGoogleState(state);
    return { purpose: parsed.purpose, nonce: parsed.nonce ?? '' };
  } catch {
    return { purpose: 'signin', nonce: '' };
  }
}
...
export function buildGoogleAuthRedirect(
  ...
  return buildGoogleAuthUrl(purpose, returnTo, nonce);
}
